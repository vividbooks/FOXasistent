  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-baseline justify-between gap-2 border-b border-zinc-200/80 pb-3">
        <p className="text-xl font-semibold tabular-nums text-zinc-900">{formatKc(e.amountKc)}</p>
      ...
      <dl className="grid gap-2 text-sm">
        <div className="flex gap-2">
          ...
          <dd className="font-medium text-zinc-900">
            {e.user.name} <span className="font-normal text-zinc-500">@{e.user.username}</span>
          </dd>
        </div>
        <div className="flex gap-2">
...
export function ExpenseReceiptExplorer({
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-lg">
      <h3 className="border-b border-zinc-100 px-4 py-3 text-sm font-medium text-zinc-900">{title}</h3>
      <div className="flex max-h-[min(85vh,900px)] min-h-[min(52vh,520px)] flex-col lg:flex-row">
        ...
        >
          <div className="p-2">
            {dayRows.map((row, i) => {
              const showWeek = i === 0 || dayRows[i - 1]!.weekLabel !== row.weekLabel;
                ...
                <div key={row.dayKey} className="mb-1">
                  ...
                  <button
                    ...
                    className={`flex w-full items-center justify-between gap-2 rounded-xl px-3 py-2.5 text-left text-sm transition ${
                      selected ? "bg-zinc-900 text-white shadow-md" : "text-zinc-900 hover:bg-zinc-200/80"
                    }`}
                  >
                    <span className="min-w-0 capitalize">{row.dayLabel}</span>
                    <span
                      className={`shrink-0 rounded-md px-1.5 py-0.5 text-xs tabular-nums ${
                        selected ? "bg-white/20 text-white" : "bg-zinc-200 text-zinc-700"
                      }`}
                    >
                      {row.items.length}
                    </span>
            ...
            <>
              ...
              <ul className="space-y-2">
                {selectedDay.items.map((e) => {
                  ...
                  return (
                    <li key={e.id} className="rounded-xl border border-zinc-200 bg-zinc-50/50">
                      <button
                        type="button"
                        aria-expanded={open}
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1">
                          <span className="flex flex-wrap items-baseline justify-between gap-2">
                            <span className="font-medium text-zinc-900">{e.user.name}</span>
