...
**Čtení účtenek a PDF** ve výchozím stavu běží **v prohlížeči** (Tesseract + PDF.js). **Gemini Vision** můžeš napojit **čistě přes Supabase** (bez Vercelu kvůli OCR): nasaď Edge Function [`supabase/functions/extract-receipt`](supabase/functions/extract-receipt/index.ts), nastav secret `GEMINI_API_KEY` (klíč z [Google AI Studio](https://aistudio.google.com/apikey)) přes `supabase secrets set GEMINI_API_KEY=…`. **Deploy z prohlížeče (GitHub Pages):** gateway s `verify_jwt = true` odmítne **CORS preflight (OPTIONS)** bez JWT — v `supabase/config.toml` je u `extract-receipt` **`verify_jwt = false`** a JWT se ověřuje v kódu funkce. Nasazení: `supabase functions deploy extract-receipt --no-verify-jwt` (nebo stejný efekt přes `config.toml` při deployi z kořene projektu). Na GitHub Pages build zapni **`VITE_USE_GEMINI_EXTRACT=1`** — aplikace volá `https://<projekt>.supabase.co/functions/v1/extract-receipt` přes `supabase.functions.invoke`. Při selhání nebo vypnutém přepínači zůstává lokální Tesseract. Volitelně má Next na Vercelu **`POST /api/extract-document`** se stejným Gemini + Tesseract fallbackem.
