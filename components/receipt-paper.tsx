...
type Props = {
  ...
  /** Částka z pole formuláře (co se uloží). */
  formAmountKc: string;
  dateLabel: string;
  loading: boolean;
  fileLabel?: string;
    ...
    <div className="relative mx-auto w-full max-w-[340px]">
      ...
      >
        ...
        <div className="px-5 pb-6 pt-4 font-mono text-[13px] leading-snug text-zinc-900">
          ...
          {loading && (
            <p className="mt-4 animate-pulse text-center text-[12px] text-zinc-500">
              Načítám text…
            </p>
          )}
          ...
          {!loading && items.length > 0 && (
            <ul className="mt-4 space-y-2.5">
              {items.map((row, i) => (
                <li key={i} className="flex gap-2 border-b border-dotted border-zinc-300/90 pb-2 last:border-0">
                  <span className="min-w-0 flex-1 break-words text-zinc-800">
                    {row.label}
                  </span>
                  <span className="shrink-0 tabular-nums text-zinc-900">
                    {formatKc(row.amountKc)}
                  </span>
                </li>
              ))}
            </ul>
